const countries = [
    ...
    //how many languages are there in the countries file
  ]
  function total(){
  let number = []
  let lang = []
  number = countries.map((num) => num.languages) //putting all languages array into numbers array
  for(let sector of number){ //iterating from numbers array
    for(let i=0; i<sector.length; i++){ //iterating from each array in numbers array to push languages into lang array
        lang.push(sector[i])
    }
    }   
    console.log(lang) //lang has now all languages as elements
    let setlang = new Set(lang) //converting lang into set()
...
//use the countries array to find the ten most spoken language
function langcount(){
    let number = []
    let lang = []
    let count = []
    let organize = []
    number = countries.map((num) => num.languages)
    for(let sector of number){
    ...
    for(let numlang of setlang){
        count = lang.filter((lng) => lng ===numlang) //filtering each language and reapeted number of times within an  array
        organize.push({language: numlang, counts: count.length}) //pushing each language and occurece in the organize array
    }
     organize.sort((a, b) => { //sorting the array
        return b.counts - a.counts
     })
      return organize.slice(0,10)//printing first ten from sorted array
}
